...
function FlipCard({
    ...
    const isSol = src.includes('solana');
    ...
    return (
        ...
        >
            ...
            >
                ...
                {/* Back Face */}
                <div
                    className={`absolute inset-0 h-full w-full overflow-hidden rounded-full shadow-lg flex flex-col items-center justify-center p-2 border-2 ${isSol ? 'bg-gradient-to-br from-[#9945FF] to-[#14F195] border-[#14F195]' : 'bg-gradient-to-br from-[#2775CA] to-[#1A5FB4] border-[#2775CA]'}`}
                    style={{ backfaceVisibility: "hidden", transform: "rotateY(180deg)" }}
                >
                    <div className="text-center">
                        <p className="text-[10px] font-bold text-white uppercase tracking-wide">{isSol ? 'SOL' : 'USDC'}</p>
                        <p className="text-[8px] font-medium text-white/80">{isSol ? 'Solana' : 'USD Coin'}</p>
                    </div>
                </div>
            </motion.div>
        </motion.div>
...
// Crypto Coin Images - USDC and SOL alternating
...
// Repeat USDC and SOL to fill the array
const IMAGES = Array.from({ length: 20 }, (_, i) => i % 2 === 0 ? SOL_LOGO : USDC_LOGO);
